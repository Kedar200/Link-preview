...
interface HeroSceneProps {
  ...
  showCursor?: boolean;
  demoApp?: 'whatsapp' | 'twitter' | 'linkedin' | 'slack' | 'discord' | 'instagram';
}

type ExportState = 'idle' | 'exporting' | 'done' | 'error';

export default function HeroScene({ onSubmit, loading, data, error, isAnimating, animState, typedUrl, buttonHighlighted, showCursor, demoApp }: HeroSceneProps) {
  ...
  const currentApp = isAnimating && demoApp ? demoApp : app;
        ...
        <div className="pointer-events-auto">
          ...
          <div className="w-full max-w-[480px]">
            <UrlInput 
              onSubmit={onSubmit} 
              loading={loading}
              animatedValue={isAnimating ? typedUrl : undefined}
        ...
        <div className="relative flex flex-col items-center justify-center">
          ...
          <div className="pointer-events-auto absolute top-[60%] sm:top-1/2 -right-8 sm:-right-12 md:-right-24 -translate-y-1/2 flex flex-col items-center gap-4 z-50 bg-[#111a15]/80 backdrop-blur-md p-2 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-white/10">
            ...
            {(() => {
              ...
              const activeIndex = Math.max(0, APPS_LIST.findIndex(a => a.id === currentApp));

              return (
                <div className="relative flex flex-col items-center gap-2 py-2">
                  <div 
                    className="absolute w-12 h-12 bg-[#dde6e1] rounded-2xl shadow-[0_10px_25px_rgba(26,43,33,0.2)] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]"
                    style={{
                      transform: `translateY(${activeIndex * 56}px) scale(1.1)`,
                      top: '8px',
                      zIndex: 0
                    }}
                  />
