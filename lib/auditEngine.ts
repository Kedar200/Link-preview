...
/**
 * Determine image format from Content-Type header, og:image:type tag, or URL extension.
 * Returns a normalised format string like "jpeg", "png", "gif", "svg", "webp", or "unknown".
 */
function detectImageFormat(data: OGData): string {
  // 1. Content-Type from HEAD request (most reliable)
  if (data.imageContentType) {
    const ct = data.imageContentType.toLowerCase();
  ...
  // 2. og:image:type tag
  if (has(data.imageType)) {
    const it = data.imageType!.toLowerCase();
    if (it.includes('jpeg') || it.includes('jpg')) return 'jpeg';
    if (it.includes('png'))  return 'png';
    if (it.includes('gif'))  return 'gif';
    if (it.includes('svg'))  return 'svg';
    if (it.includes('webp')) return 'webp';
  }
  // 3. URL extension fallback
  if (has(data.image)) {
    try {
      const pathname = new URL(data.image).pathname.toLowerCase();
...
export function runAudit(data: OGData): AuditResult {
  ...
  // ── og:title ───────────────────────────────────────────────────────────
  if (!has(data.title)) {
    findings.push({ id: next(), tag: 'og:title', severity: 'critical', plain: 'Your page has no title — links will look blank on every platform.', technical: 'og:title is missing', platforms: ['whatsapp', 'twitter', 'linkedin', 'slack', 'discord', 'instagram', 'facebook'], fix: `<meta property="og:title" content="Your Page Title" />` });
  } else {
    ...
    if (charLen(data.title) < 15) {
      findings.push({ id: next(), tag: 'og:title', severity: 'info', plain: 'Your title is quite short — consider adding more context for better engagement.', technical: `og:title only ${charLen(data.title)} chars`, platforms: ['whatsapp', 'twitter', 'linkedin', 'slack', 'discord', 'instagram', 'facebook'] });
    }
    findings.push({ id: next(), tag: 'og:title', severity: 'pass', plain: 'Title is present and well-formatted.', technical: `og:title: "${data.title.slice(0, 50)}${data.title.length > 50 ? '…' : ''}"`, platforms: ['whatsapp', 'twitter', 'linkedin', 'slack', 'discord', 'instagram', 'facebook'] });
  }

  // ── og:description ─────────────────────────────────────────────────────
  if (!has(data.description)) {
    findings.push({ id: next(), tag: 'og:description', severity: 'critical', plain: 'No description found — previews will show a blank subtitle or auto-generate from your page content.', technical: 'og:description is missing', platforms: ['whatsapp', 'twitter', 'linkedin', 'slack', 'discord', 'instagram', 'facebook'], fix: `<meta property="og:description" content="A brief summary of your page." />` });
  } else {
    if (charLen(data.description) > 200) {
      findings.push({ id: next(), tag: 'og:description', severity: 'warning', plain: `Your description is ${charLen(data.description)} characters — most platforms only show the first 150-200.`, technical: `Description is ${charLen(data.description)} chars`, platforms: ['twitter', 'linkedin', 'instagram', 'facebook'] });
    }
    if (charLen(data.description) < 50) {
      findings.push({ id: next(), tag: 'og:description', severity: 'info', plain: 'Your description is a bit short. Add more detail to entice clicks.', technical: `og:description only ${charLen(data.description)} chars`, platforms: ['whatsapp', 'twitter', 'linkedin', 'slack', 'discord', 'instagram', 'facebook'] });
    }
    findings.push({ id: next(), tag: 'og:description', severity: 'pass', plain: 'Description looks good.', technical: `og:description present (${charLen(data.description)} chars)`, platforms: ['whatsapp', 'twitter', 'linkedin', 'slack', 'discord', 'instagram', 'facebook'] });
  }

  // ── og:image ───────────────────────────────────────────────────────────
  if (!has(data.image)) {
    findings.push({ id: next(), tag: 'og:image', severity: 'critical', plain: 'No preview image! Your link will show as a plain text card — people are 3× less likely to click.', technical: 'og:image is missing', platforms: ['whatsapp', 'twitter', 'linkedin', 'slack', 'discord', 'instagram', 'facebook'], fix: `<meta property="og:image" content="https://yoursite.com/preview.jpg" />` });
  } else {
    // Check if relative URL
    if (!data.image.startsWith('http')) {
      findings.push({ id: next(), tag: 'og:image', severity: 'warning', plain: 'Your image URL is relative — some platforms like WhatsApp need an absolute URL starting with https://.', technical: 'og:image is a relative URL', platforms: ['whatsapp', 'instagram', 'linkedin', 'facebook'] });
    }
    // HTTPS check
    if (data.image.startsWith('http://')) {
  ...
  // ── og:type ────────────────────────────────────────────────────────────
  if (!has(data.type)) {
    findings.push({ id: next(), tag: 'og:type', severity: 'info', plain: 'No content type specified — platforms default to "website" but it\'s better to be explicit.', technical: 'og:type is missing', platforms: ['instagram', 'facebook'], fix: `<meta property="og:type" content="website" />` });
  } else {
    findings.push({ id: next(), tag: 'og:type', severity: 'pass', plain: `Content type is set to "${data.type}".`, technical: `og:type = ${data.type}`, platforms: ['instagram', 'facebook'] });
  }

  // ── og:url ─────────────────────────────────────────────────────────────
  if (!has(data.url)) {
    findings.push({ id: next(), tag: 'og:url', severity: 'info', plain: 'Canonical URL isn\'t set — set og:url so shares always point to the right page.', technical: 'og:url is missing', platforms: ['instagram', 'linkedin', 'facebook'], fix: `<meta property="og:url" content="https://yoursite.com/page" />` });
  } else {
    findings.push({ id: next(), tag: 'og:url', severity: 'pass', plain: 'Canonical URL is set.', technical: `og:url present`, platforms: ['instagram', 'linkedin', 'facebook'] });
  }

  // ── og:site_name ───────────────────────────────────────────────────────
  if (!has(data.siteName)) {
    findings.push({ id: next(), tag: 'og:site_name', severity: 'info', plain: 'Site name is missing — add it so Instagram and LinkedIn show your brand name.', technical: 'og:site_name is missing', platforms: ['instagram', 'linkedin', 'facebook'], fix: `<meta property="og:site_name" content="Your Site Name" />` });
  } else {
    findings.push({ id: next(), tag: 'og:site_name', severity: 'pass', plain: `Site name: "${data.siteName}"`, technical: 'og:site_name present', platforms: ['instagram', 'linkedin', 'facebook'] });
  }
  ...
  if (!has(data.twitterSite)) {
    findings.push({ id: next(), tag: 'twitter:site', severity: 'info', plain: 'No twitter:site tag — add your @username so your account is credited when your link is shared on X.', technical: 'twitter:site is missing', platforms: ['twitter'], fix: `<meta name="twitter:site" content="@yourusername" />` });
  } else {
    findings.push({ id: next(), tag: 'twitter:site', severity: 'pass', plain: `Twitter site attribution: ${data.twitterSite}`, technical: `twitter:site = ${data.twitterSite}`, platforms: ['twitter'] });
  }
  ...
  // WhatsApp image file size: 600 KB limit (images over 600 KB show NO preview image)
  if (has(data.image) && data.imageSize != null && data.imageSize > 600 * 1024) {
    const sizeKB = Math.round(data.imageSize / 1024);
    findings.push({ id: next(), tag: 'og:image', severity: 'critical', plain: `Your OG image is ${sizeKB} KB — WhatsApp won't load images larger than 600 KB. The preview will appear without an image.`, technical: `og:image file size ${sizeKB} KB exceeds WhatsApp 600 KB limit`, platforms: ['whatsapp'] });
  }
  ...
  if (has(data.image) && has(data.imageWidth) && has(data.imageHeight)) {
    ...
    if (!isNaN(w) && !isNaN(h)) {
      if (w < 100 || h < 100) {
        ...
        findings.push({ id: next(), tag: 'og:image', severity: 'info', plain: `Your image is ${w}px wide — WhatsApp will show a small thumbnail. Use ≥300px width for a full-width preview card.`, technical: `Image width ${w}px → small thumbnail (need ≥300px for large preview)`, platforms: ['whatsapp'] });
      }
    }
  }

  // WhatsApp image format check: GIF and SVG NOT supported
  if (has(data.image)) {
    const format = detectImageFormat(data);
    if (format === 'gif') {
      findings.push({ id: next(), tag: 'og:image', severity: 'warning', plain: 'Your image is a GIF — WhatsApp does not support GIF for link previews. Use JPG, PNG, or WebP instead.', technical: 'GIF format not supported by WhatsApp', platforms: ['whatsapp'] });
    }
    if (format === 'svg') {
      findings.push({ id: next(), tag: 'og:image', severity: 'warning', plain: 'Your image is an SVG — WhatsApp and Twitter do not support SVG for link previews. Use JPG or PNG.', technical: 'SVG format not supported by WhatsApp or Twitter', platforms: ['whatsapp', 'twitter'] });
    }
  }
  ...
  if (has(data.image)) {
    ...
    // Facebook expanded vs compact preview thresholds
    if (!isNaN(w) && !isNaN(h) && w >= 200 && h >= 200) {
      const ratio = w / h;
      if (w <= 445 || h <= 232 || ratio <= 0.797) {
        findings.push({ id: next(), tag: 'og:image', severity: 'info', plain: 'Your image will show as a compact thumbnail on Facebook. For a full-width preview, use an image wider than 445px, taller than 232px, with aspect ratio above 0.8.', technical: `Facebook compact preview: width=${w}, height=${h}, ratio=${ratio.toFixed(2)}`, platforms: ['facebook'] });
      }
    }

    // Facebook file size: 8 MB max
    if (data.imageSize != null && data.imageSize > 8 * 1024 * 1024) {
      const sizeMB = (data.imageSize / (1024 * 1024)).toFixed(1);
      findings.push({ id: next(), tag: 'og:image', severity: 'warning', plain: `Your image is ${sizeMB} MB — Facebook's maximum is 8 MB.`, technical: `og:image ${sizeMB} MB exceeds Facebook 8 MB limit`, platforms: ['facebook'] });
    }
  }

  // Facebook title truncation
  if (has(data.title) && charLen(data.title) > 88) {
    findings.push({ id: next(), tag: 'og:title', severity: 'info', plain: 'Facebook truncates titles after roughly 88 characters.', technical: 'Title too long for Facebook', platforms: ['facebook'] });
  }

  // ── Instagram DM-specific checks ───────────────────────────────────────
  if (has(data.image)) {
    // Instagram DM file size: 8 MB max
    if (data.imageSize != null && data.imageSize > 8 * 1024 * 1024) {
      const sizeMB = (data.imageSize / (1024 * 1024)).toFixed(1);
      findings.push({ id: next(), tag: 'og:image', severity: 'warning', plain: `Your image is ${sizeMB} MB — Instagram DMs won't display images larger than 8 MB.`, technical: `og:image ${sizeMB} MB exceeds Instagram DM 8 MB limit`, platforms: ['instagram'] });
    }
  ...
  // ── Multiple og:image tags warning ─────────────────────────────────────
  if (data.multipleOgImages) {
    findings.push({ id: next(), tag: 'og:image', severity: 'warning', plain: 'Your page has multiple og:image tags — this causes unpredictable behaviour. Most platforms use the first one, but results may vary. Use only one og:image tag.', technical: 'Multiple og:image meta tags detected', platforms: ['whatsapp', 'twitter', 'linkedin', 'slack', 'discord', 'instagram', 'facebook'] });
  }

  // ── General best-practice checks ───────────────────────────────────────
  if (has(data.title) && data.title === data.title.toUpperCase() && data.title.length > 5) {
    findings.push({ id: next(), tag: 'og:title', severity: 'info', plain: 'Your title is ALL CAPS — this looks spammy on social platforms and hurts click-through rates.', technical: 'Title is entirely uppercase', platforms: ['whatsapp', 'twitter', 'linkedin', 'slack', 'discord', 'instagram', 'facebook'] });
  }
  ...
  const platformNames = ['whatsapp', 'twitter', 'linkedin', 'slack', 'discord', 'instagram', 'facebook'] as const;
  const platformIcons: Record<string, string> = {
    whatsapp: '💬', twitter: '𝕏', linkedin: '💼', slack: '⚡', discord: '🎮', instagram: '📷', facebook: '📘',
  };
